...
pub struct GameState {
    ...
    pub current_player: bool, // False for human (X), True for computer (O)
}
...
pub fn play_computer_move(state: &GameState, nn: &mut NeuralNetwork, display_move: bool) -> usize {
...
pub fn is_game_over(state: &GameState, winner: &mut char) -> bool {
    // Check the rows
...
pub fn train_against_random(nn: &mut NeuralNetwork, num_games: u32) {
    println!(
        "\nTraining neural network with one hidden layer against {} random games...",
        num_games
    );
